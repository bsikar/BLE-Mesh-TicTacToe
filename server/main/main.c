...
#define GATTS_TAG "GATTS_MSPLAYER"
...
#define CLIENT_DEVICE_NAME           "ESP_MSPLAYER"
#define CLIENT_MANUFACTURER_DATA_LEN 17
...
static uint8_t raw_adv_data[] = {
    ...
    0x02, 0x0a,
    0xeb, // Length 2, Data Type 10 (TX power leve), Data 2 (-21)
    0x03, 0x03, 0xab,
    0xcd, // Length 3, Data Type 3 (Complete 16-bit Service UUIDs), Data 3
          // (UUID)
};
static uint8_t raw_scan_rsp_data[] =
    { // Length 15, Data Type 9 (Complete Local Name), Data 1 (ESP_MSPLAYER)
...
// static uint8_t test_manufacturer[CLIENT_MANUFACTURER_DATA_LEN] =  {0x12,
// 0x23, 0x45, 0x56}; adv data
static esp_ble_adv_data_t adv_data = {
    ...
    .appearance          = 0x00,
    .manufacturer_len    = 0,    // CLIENT_MANUFACTURER_DATA_LEN,
...
#define PROFILE_NUM  1 /* how many profiles there are */
#define PROFILE_A_ID 0 /* can have more id's for more profiles */
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t      event,
  ...
  case ESP_GATTS_REG_EVT:
    ...
    gl_profile_tab[PROFILE_A_ID].service_id.is_primary  = true;
    gl_profile_tab[PROFILE_A_ID].service_id.id.inst_id  = 0x00;
    gl_profile_tab[PROFILE_A_ID].service_id.id.uuid.len = ESP_UUID_LEN_16;
    gl_profile_tab[PROFILE_A_ID].service_id.id.uuid.uuid.uuid16 =
        GATTS_SERVICE_UUID;

    esp_err_t set_dev_name_ret = esp_bt_dev_set_device_name(CLIENT_DEVICE_NAME);
    if (set_dev_name_ret) {
    ...
    esp_ble_gatts_create_service(
        gatts_if, &gl_profile_tab[PROFILE_A_ID].service_id, GATTS_NUM_HANDLE);
    break;
  ...
  case ESP_GATTS_WRITE_EVT: {
    ...
    if (!param->write.is_prep) {
      ...
      if (gl_profile_tab[PROFILE_A_ID].descr_handle == param->write.handle &&
          param->write.len == 2) {
        ...
        if (descr_value == 0x0001) {
          if (a_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY) {
            ...
            esp_ble_gatts_send_indicate(
                ...
                gl_profile_tab[PROFILE_A_ID].char_handle, sizeof(notify_data),
                notify_data, false);
          }
          ...
          if (a_property & ESP_GATT_CHAR_PROP_BIT_INDICATE) {
            ...
            esp_ble_gatts_send_indicate(
                gatts_if, param->write.conn_id,
                gl_profile_tab[PROFILE_A_ID].char_handle, sizeof(indicate_data),
                indicate_data, true);
          }
  ...
  case ESP_GATTS_CREATE_EVT:
    ...
    gl_profile_tab[PROFILE_A_ID].service_handle = param->create.service_handle;
    gl_profile_tab[PROFILE_A_ID].char_uuid.len  = ESP_UUID_LEN_16;
    gl_profile_tab[PROFILE_A_ID].char_uuid.uuid.uuid16 = GATTS_CHAR_UUID;

    esp_ble_gatts_start_service(gl_profile_tab[PROFILE_A_ID].service_handle);
    a_property = ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_WRITE |
                 ESP_GATT_CHAR_PROP_BIT_NOTIFY;
    esp_err_t add_char_ret =
        esp_ble_gatts_add_char(gl_profile_tab[PROFILE_A_ID].service_handle,
                               &gl_profile_tab[PROFILE_A_ID].char_uuid,
  ...
  case ESP_GATTS_ADD_CHAR_EVT: {
    ...
    gl_profile_tab[PROFILE_A_ID].char_handle    = param->add_char.attr_handle;
    gl_profile_tab[PROFILE_A_ID].descr_uuid.len = ESP_UUID_LEN_16;
    gl_profile_tab[PROFILE_A_ID].descr_uuid.uuid.uuid16 =
    ...
    esp_err_t add_descr_ret = esp_ble_gatts_add_char_descr(
        gl_profile_tab[PROFILE_A_ID].service_handle,
        &gl_profile_tab[PROFILE_A_ID].descr_uuid,
  ...
  case ESP_GATTS_ADD_CHAR_DESCR_EVT:
    gl_profile_tab[PROFILE_A_ID].descr_handle =
  ...
  case ESP_GATTS_CONNECT_EVT: {
    ...
    gl_profile_tab[PROFILE_A_ID].conn_id = param->connect.conn_id;
