...
#define GATTC_TAG               "GATTC_MSPLAYER"
...
static const char              remote_device_name[] = "ESP_MSPLAYER";
static bool                    connect              = false;
static bool                    get_server           = false;
static esp_gattc_char_elem_t  *char_elem_result     = NULL;
static esp_gattc_descr_elem_t *descr_elem_result    = NULL;
static esp_gatt_if_t           static_gattc_if;
    ...
    if (get_server) {
      ...
      if (count > 0) {
        ...
        if (!char_elem_result) {
          ...
          /*  Every service have only one char in our 'ESP_MSPLAYER' demo, so
    ...
    if (p_data->reg_for_notify.status != ESP_GATT_OK) {
      ...
      if (count > 0) {
        ...
        if (!descr_elem_result) {
          ...
          /* Every char has only one descriptor in our 'ESP_MSPLAYER' demo, so
...
void write_gpio_pin_number(uint32_t gpio_num) {
  char message[20]; // Adjust size based on expected server input
  snprintf(message, sizeof(message), "%ld", gpio_num);

  esp_err_t status = esp_ble_gattc_write_char(
      static_gattc_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id,
      gl_profile_tab[PROFILE_A_APP_ID].char_handle, sizeof(message),
      (uint8_t *)message, ESP_GATT_WRITE_TYPE_RSP, ESP_GATT_AUTH_REQ_NONE);

  if (status != ESP_OK) {
    ESP_LOGE("BLE", "Failed to write characteristic: %s",
             esp_err_to_name(status));
  } else {
    ESP_LOGI("BLE", "Successfully sent GPIO pin number %ld to server",
             gpio_num);
  }
}
...
static void gpio_tasks(void *arg) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(gpio_evt_queue, &io_num,
      ...
      // Here, send a BLE message to the server
      write_gpio_pin_number(io_num);
    }
  }
}

void app_main(void) {
  /* Initialize BLE and set up GATT client */
  ...
  xTaskCreate(gpio_tasks, "gpio_tasks", 2048, NULL, 10, NULL);
